...
!!
  ...
  subroutine output_hamiltonian(outp, namespace, dir, hm, st, der, geo, gr, grp)
    ...
    integer :: is, err, idir, ik, ib, iout
    ...
    do iout = 1, size(outp%what)
      if(bitand(outp%what(iout), OPTION__OUTPUT__POTENTIAL) /= 0) then
        ...
        call dio_function_output(outp%how(iout), dir, "v0", namespace, &
          der%mesh, v0(:, 1), units_out%energy, err, geo = geo, grp = grp)
        SAFE_DEALLOCATE_A(v0)

        if(hm%ep%classical_pot > 0) then
          call dio_function_output(outp%how(iout), dir, "vc", namespace, &
            der%mesh, hm%ep%Vclassical, units_out%energy, err, geo = geo, grp = grp)
        end if

        if(associated(hm%ep%v_static)) then
          call dio_function_output(outp%how(iout), dir, "vext", namespace, &
            der%mesh, hm%ep%v_static, units_out%energy, err, geo = geo, grp = grp)
        end if

        if(hm%theory_level /= INDEPENDENT_PARTICLES) then
          call dio_function_output(outp%how(iout), dir, 'vh', namespace, &
            der%mesh, hm%vhartree, units_out%energy, err, geo = geo, grp = grp)
          if(bitand(outp%what(iout), OPTION__OUTPUT__POTENTIAL_GRADIENT) /= 0) then
            ...
            call io_function_output_vector(outp%how(iout), dir, 'grad_vh', namespace, &
            ...
            call io_function_output_vector(outp%how(iout), dir, 'grad_v0', namespace, &
              der%mesh, gradvh(:, :), der%mesh%sb%dim, units_out%force, err,&
                    geo = geo, grp = grp, vector_dim_labels = (/'x', 'y', 'z'/))
            SAFE_DEALLOCATE_A(v0)
          ...
          do is = 1, min(hm%d%ispin, 2)
            ...
            call dio_function_output(outp%how(iout), dir, fname, namespace, &
            ...
            if (hm%ep%classical_pot > 0) then
              call dio_function_output(outp%how(iout), dir, fname, namespace, &
                der%mesh, potential + hm%ep%Vclassical, units_out%energy, err, geo = geo, grp = grp)
            else
              call dio_function_output(outp%how(iout), dir, fname, namespace, &
                der%mesh, potential, units_out%energy, err, geo = geo, grp = grp)
            end if
          end do
        ...
        if(hm%theory_level == KOHN_SHAM_DFT .and. hm%pcm%run_pcm ) then
          if (hm%pcm%solute .and. hm%pcm%localf) then
            call dio_function_output(outp%how(iout), dir, 'vpcm', namespace, &
              der%mesh, hm%pcm%v_e_rs + hm%pcm%v_n_rs + hm%pcm%v_ext_rs , &
              units_out%energy, err, geo = geo, grp = grp)
            call dio_function_output(outp%how(iout), dir, 'vpcm_sol', namespace, &
              der%mesh, hm%pcm%v_e_rs + hm%pcm%v_n_rs , &
              units_out%energy, err, geo = geo, grp = grp)
            call dio_function_output(outp%how(iout), dir, 'vpcm_e', namespace, &
              der%mesh, hm%pcm%v_e_rs , &
              units_out%energy, err, geo = geo, grp = grp)
            call dio_function_output(outp%how(iout), dir, 'vpcm_n', namespace, &
              der%mesh, hm%pcm%v_n_rs , &
              units_out%energy, err, geo = geo, grp = grp)
            call dio_function_output(outp%how(iout), dir, 'vpcm_ext', namespace, &
        ...
        if(hm%self_induced_magnetic) then
          ...
          case(3)
            do idir = 1, der%mesh%sb%dim
              call dio_function_output(outp%how(iout), dir, 'Bind_'//index2axis(idir), namespace, &
                der%mesh, hm%b_ind(:, idir), &
                units_out%force, err, geo = geo, grp = grp)
            end do
          case(2)
            call dio_function_output(outp%how(iout), dir, 'Bind_z', namespace, &
              der%mesh, hm%b_ind(:, 1), units_out%force, err, geo = geo, grp = grp)
          end select
        end if
      end if


      if(bitand(outp%what(iout), OPTION__OUTPUT__XC_DENSITY) /= 0 .and. hm%theory_level /= INDEPENDENT_PARTICLES) then
        SAFE_ALLOCATE(v0(1:der%mesh%np_part, 1))
        ...
        do is = 1, min(hm%d%ispin, 2)
          ...
          call dio_function_output(outp%how(iout), dir, fname, namespace, &
            der%mesh, nxc, units_out%energy, err, geo = geo, grp = grp)
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__CURRENT) /= 0) then
        ...
        if(states_are_complex(st)) then
          ...
          do is = 1, hm%d%nspin
            ...
            call io_function_output_vector(outp%how(iout), dir, fname, namespace, der%mesh, &
      ...
      if(bitand(outp%whatBZ, OPTION__OUTPUT_KPT__CURRENT_KPT) /= 0) then
        ...
        if(states_are_complex(st)) then
          ...
          call io_function_output_vector_BZ(outp%how(iout), dir, fname, namespace, der%mesh, st%d%kpt, &
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__HEAT_CURRENT) /= 0) then
      ...
      if(bitand(outp%whatBZ, OPTION__OUTPUT_KPT__DENSITY_KPT) /= 0) then
        ...
        call io_function_output_global_BZ(outp%how(iout), dir, "density_kpt", namespace, &
  ...
  ! ---------------------------------------------------------
  subroutine output_scalar_pot(outp, namespace, dir, gr, geo, hm, time)
    type(output_t),           intent(in)    :: outp
    type(namespace_t),        intent(in)    :: namespace
    character(len=*),         intent(in)    :: dir
    type(grid_t),             intent(in)    :: gr
    ...
    do iout = 1, size(outp%what)
      if(bitand(outp%what(iout), OPTION__OUTPUT__EXTERNAL_TD_POTENTIAL) /= 0) then
  ...
  ! ---------------------------------------------------------
  subroutine output_kick(outp, namespace, dir, mesh, geo, kick)
    type(output_t),    intent(in) :: outp
    type(namespace_t), intent(in) :: namespace
    character(len=*),  intent(in) :: dir
    ...
    do iout = 1, size(outp%what)
      if(bitand(outp%what(iout), OPTION__OUTPUT__DELTA_PERTURBATION) /= 0) then
        SAFE_ALLOCATE(kick_function(1:mesh%np))
        call kick_function_get(mesh, kick, kick_function, 1)
        call zio_function_output(outp%how(iout), dir, "kick_function", namespace, mesh, kick_function(:), &
