...
module output_oct_m
  ...
  type output_t
    ...
    integer(8), pointer, public :: what(:) => null()     !< what to output
    ...
    integer(8), pointer, public :: how(:) => null()      !< how to output
...
contains
  ...
  subroutine output_init(outp, namespace, sb, st, nst, ks, states_are_real)
    ...
    if(parse_block(namespace, 'Output', blk) == 0) then
      ...
      if(ncols == 2) then
        ...
        SAFE_ALLOCATE(outp%what(1:nrows))
      ...
      !old format
      SAFE_ALLOCATE(outp%what(1))
      SAFE_ALLOCATE(outp%how(1))
      call parse_variable(namespace, 'Output', 0, outp%what(1))
    endif

    do iout = 1, size(outp%what)
      if(bitand(outp%what(iout), OPTION__OUTPUT__WFS_FOURIER) /= 0) then
        call messages_experimental("Wave-functions in Fourier space")
      end if

      ! cannot calculate the ELF in 1D
      if(bitand(outp%what(iout), OPTION__OUTPUT__ELF) /= 0 .or. bitand(outp%what(iout), OPTION__OUTPUT__ELF_BASINS) /= 0) then
        if(sb%dim /= 2 .and. sb%dim /= 3) then
          outp%what(iout) = bitand(outp%what(iout), not(OPTION__OUTPUT__ELF + OPTION__OUTPUT__ELF_BASINS))
          write(message(1), '(a)') 'Cannot calculate ELF except in 2D and 3D.'
          call messages_warning(1, namespace=namespace)
        end if
      end if

      if(.not.varinfo_valid_option('Output', outp%what(iout), is_flag=.true.)) then
        call messages_input_error('Output')
      end if

      if(bitand(outp%what(iout), OPTION__OUTPUT__MMB_WFS) /= 0) then
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__ENERGY_DENSITY) /= 0) call messages_experimental("'Output = energy_density'")
      if(bitand(outp%what(iout), OPTION__OUTPUT__HEAT_CURRENT) /= 0) call messages_experimental("'Output = heat_current'")
      
      if(bitand(outp%what(iout), OPTION__OUTPUT__WFS) /= 0  .or.  bitand(outp%what(iout), OPTION__OUTPUT__WFS_SQMOD) /= 0 ) then
      ...
      if(parse_block(namespace, 'CurrentThroughPlane', blk) == 0) then
        outp%what(iout) = ior(outp%what(iout), OPTION__OUTPUT__J_FLOW)
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__MATRIX_ELEMENTS) /= 0) then
        call output_me_init(outp%me, namespace, sb, st, nst)
      else
        outp%me%what = 0
      end if

      if(bitand(outp%what(iout), OPTION__OUTPUT__BERKELEYGW) /= 0) then
    ...
    !% Outputs the electronic density resolved in momentum space. 
     ...
     call v_ks_calculate_current(ks, .true.) 
      ...
      if(outp%what(iout) + outp%whatBZ + outp%what_lda_u /= 0 .and. outp%output_interval > 0) then
  ...
  subroutine output_all(outp, namespace, dir, gr, geo, st, hm, ks)
    ...
    do iout = 1, size(outp%what)
      if(outp%what(iout)+outp%whatBZ+outp%what_lda_u /= 0) then
        message(1) = "Info: Writing output to " // trim(dir)
        call messages_info(1)
        call io_mkdir(dir, namespace)
      end if

      if(bitand(outp%what(iout), OPTION__OUTPUT__MESH_R) /= 0) then
        do idir = 1, gr%mesh%sb%dim
          write(fname, '(a,a)') 'mesh_r-', index2axis(idir)
          call dio_function_output(outp%how(iout), dir, fname, namespace, gr%mesh, gr%mesh%x(:,idir), &
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__GEOMETRY) /= 0) then
        if(bitand(outp%how(iout), OPTION__OUTPUTFORMAT__XCRYSDEN) /= 0) then        
          call write_xsf_geometry_file(dir, "geometry", geo, gr%mesh, namespace)
        end if
        if(bitand(outp%how(iout), OPTION__OUTPUTFORMAT__XYZ) /= 0) then
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__FORCES) /= 0) then
        if(bitand(outp%how(iout), OPTION__OUTPUTFORMAT__BILD) /= 0) then
      ...
      if (bitand(ks%xc_family, XC_FAMILY_OEP) /= 0 .and. ks%theory_level /= HARTREE_FOCK) then
        if (ks%oep%level == XC_OEP_FULL) then
          if (ks%oep%has_photons) then
            if(bitand(outp%what(iout), OPTION__OUTPUT__PHOTON_CORRELATOR) /= 0) then
              write(fname, '(a)') 'photon_correlator'
              call dio_function_output(outp%how(iout), dir, trim(fname), namespace, gr%mesh, ks%oep%pt%correlator(:,1), &
                units_out%length, ierr, geo = geo)
            end if
          end if
        end if
  ...
  subroutine output_localization_funct(outp, namespace, dir, st, hm, gr, geo)
    ...
    do iout = 1, size(outp%what)
      ...
      if(bitand(outp%what(iout), OPTION__OUTPUT__ELF) /= 0 .or. bitand(outp%what(iout), OPTION__OUTPUT__ELF_BASINS) /= 0) then
        ...
        if(bitand(outp%what(iout), OPTION__OUTPUT__ELF) /= 0) then
          write(fname, '(a)') 'elf_rs'
          call dio_function_output(outp%how(iout), dir, trim(fname), namespace, gr%mesh, &
        ...
        if(bitand(outp%what(iout), OPTION__OUTPUT__ELF_BASINS) /= 0) &
          call out_basins(f_loc(:,1), "elf_rs_basins")
      end if

      ! Now Bader analysis
      if(bitand(outp%what(iout), OPTION__OUTPUT__BADER) /= 0) then
        do is = 1, st%d%nspin
      ...
      ! Now the pressure
      if(bitand(outp%what(iout), OPTION__OUTPUT__EL_PRESSURE) /= 0) then
        call calc_electronic_pressure(st, hm, gr, f_loc(:,1))
        call dio_function_output(outp%how(iout), dir, "el_pressure", namespace, gr%mesh, &
  ...
  contains
    ...
    subroutine out_basins(ff, filename)
      ...
      do iout = 1, size(outp%what)
        call dio_function_output(outp%how(iout), dir, trim(filename), namespace, gr%mesh, &
  ...
  subroutine output_energy_density(outp, namespace, dir, hm, ks, st, der, geo, gr, grp)
    ...
    do iout = 1, size(outp%what)
      if(bitand(outp%what(iout), OPTION__OUTPUT__ENERGY_DENSITY) /= 0) then
...
!      end if
  ...
  logical function output_need_exchange(outp) result(need_exx)
    ...
    do iout = 1, size(outp%what)
      need_exx =( bitand(outp%what(iout), OPTION__OUTPUT__BERKELEYGW) /= 0 &
  ...
  subroutine output_dftu_orbitals(outp, dir, namespace, this, st, mesh, geo, has_phase)
    ...
    integer :: ios, im, ik, idim, ierr, iout
          ...
          do idim = 1, st%d%dim
            ...
            do iout = 1, size(outp%what)
              if(has_phase) then
                ...
                call zio_function_output(outp%how(iout), dir, fname, namespace, mesh, &
                ...
                call zio_function_output(outp%how(iout), dir, fname, namespace, mesh, tmp, fn_unit, ierr, geo = geo)
                end if
              else
                if(this%basisfromstates) then
                  if (states_are_real(st)) then
                    call dio_function_output(outp%how(iout), dir, fname, namespace, mesh, &
                  ...
                  if (states_are_real(st)) then
                    ...
                    call dio_function_output(outp%how(iout), dir, fname, namespace, mesh, dtmp, fn_unit, ierr, geo = geo)
                  else
                    tmp = M_Z0
  ...
  logical function output_needs_current(outp, states_are_real)
    ...
    do iout = 1, size(outp%what)
      if( bitand(outp%what(iout), OPTION__OUTPUT__CURRENT) /= 0 &
      .or. bitand(outp%what(iout), OPTION__OUTPUT__HEAT_CURRENT) /= 0 &
